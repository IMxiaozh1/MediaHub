...
class PlayerPresenter final : public QObject {
 ...
 private:
  ...
  void handleNetworkActivityCheck();
  void handleNetworkActivityStalled();
  void rememberNetworkUrl(const QString& url);
  void scheduleNetworkReconnect(const char* reason);
  void resetNetworkReconnect();
  void startNetworkActivityMonitoring();
  void resetNetworkActivityMonitoring();
  void openCurrentPlaylistItem(bool isAutomaticReconnect = false);
  void render();
  ...
  QTimer* networkActivityPollTimer_{nullptr};
  QTimer* networkActivityWatchdogTimer_{nullptr};
  ...
  std::optional<core::NetworkStreamActivity> lastNetworkActivity_;

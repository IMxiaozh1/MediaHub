...
GeneratedWav::~GeneratedWav() { removeDirectory(directory_); }

std::string GeneratedWav::source() const { return pathToUtf8(path_); }
...
void GeneratedWav::writeFile(const std::chrono::milliseconds duration) {
  ...
  const auto durationCount =
      std::max(duration.count(), std::chrono::milliseconds::rep{0});
  const auto sampleCount =
      static_cast<std::uint64_t>(durationCount) * kSampleRate / 1'000;
  constexpr auto kMaximumDataSize =
  ...
  constexpr std::uint32_t kHalfPeriodSamples = kSampleRate / (440U * 2U);
  constexpr std::int16_t kToneAmplitude = 12'000;
  for (std::uint64_t index = 0; index < sampleCount; ++index) {
    const std::int16_t sample = (index / kHalfPeriodSamples) % 2U == 0U
                                    ? kToneAmplitude
                                    : -kToneAmplitude;
    writeLittleEndian16(output, static_cast<std::uint16_t>(sample));
  }
  if (!output.good()) {
...
GeneratedInvalidMedia::~GeneratedInvalidMedia() { removeDirectory(directory_); }

std::string GeneratedInvalidMedia::source() const { return pathToUtf8(path_); }
